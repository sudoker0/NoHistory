...
let customUrlList: string[] = [];
let customPatternList: Pattern[] = [];
...
var history_entry_list_to_clean: browser.history.HistoryItem[] = []
...
search.onclick = async () => {
    ...
    //@ts-ignore
    if (isNaN(to)) { to = dayjs().unix(); }
    worker.onmessage = (e) => {
        var b = e.data as { total: number, output: browser.history.HistoryItem[] }
        var position = 0;
        ...
        while (matched_table.firstChild) { matched_table.removeChild(matched_table.lastChild) }
        ...
        getNewHistoryEntry();
        if (b.output.length > 0) clean["disabled"] = false;
        ...
        var lastEntryObserver = new IntersectionObserver(entries => {
            ...
            if (!!last_history_entry) { lastEntryObserver.observe(last_history_entry) }
        }, {
            rootMargin: "100px",
            root: qSel("#table_wrapper")
        })
...
clean.onclick = async () => {
    if (!confirm("Are you sure you want to clean the history?")) return;
