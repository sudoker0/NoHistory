onmessage = async (e: MessageEvent<[string[], Pattern[], browser.history.HistoryItem[], number, number]>) => {
    console.log("test")
    var output = []
    ...
    all_history.forEach(v => {
        if (v.lastVisitTime < from || to < v.lastVisitTime) return;
        if (urlList.indexOf(new URL(v.url).hostname) >= 0 || patternList.filter(f => {
            ...
            //browser.history.deleteUrl({ url: v.url });\
            // console.log(v.url)
            output.push(v)
        }
    })
